...
function ComparativeGrapihc() {
  ...
  //const {data : comparativeData, isLoading: loadingComparative, isError: errorComparative} = useComparativeData(filter)
    ...
    <section className=" relative flex bg-slate-200 rounded-md w-full h-auto p-6 sm:py-9 sm:px-12 flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 items-center justify-items-center">
        <BarGrapihc
          ...
          color2="--color-primary-1"
        />
        <BarGrapihc
          title="Gastos"
          data={chartData2}
          config={chartConfig}
          color1="--color-expense"
          color2="--color-expense-1"
        />
        <BarGrapihc
          title="Balance"
          data={chartData}
